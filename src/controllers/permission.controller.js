...
const getPermissions = async (req, res, next) => {
  try {
    ...
    if (!results) throw new NotFoundError("Error geting permissions.");
...
const getPermissionById = async (req, res, next) => {
  try {
    ...
    if (!permission) throw new NotFoundError("Error geting permission.");
...
const updatePermission = async (req, res, next) => {
  try {
    ...
    if (!permission) throw new NotFoundError("Error updating permission");
    response(res, 200, permission);
  } catch (error) {
    next(error);
  }
...
const deletePermission = async (req, res, next) => {
  try {
    ...
    if (!result) throw new NotFoundError("Error deleting permission.");
    res.status(204).end();
  } catch (error) {
    next(error);
  }
